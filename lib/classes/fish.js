...
class Fish {
  ...
  async checkForFish(client, message, permissionsBitfield) {
    ...
    try {
      ...
      // Silent fail - network errors, timeouts, etc. won't crash the bot
      // Optionally log to console if debugging is needed:
      // console.error("[anti-phish] API request failed:", error.message);
    }
  }
}
